...
function wbk_render_tool(tool){
	...
	jQuery('#days_container').html('<div class="loading"></div>');
 	jQuery.post(ajaxurl, data, function( response ) {
 		...
 		jQuery( '#lock_date_range' ).datepick( {rangeSelect: true, monthsToShow: 3} );	 
 		jQuery( '#lock_exclude_date' ).datepick( {multiSelect: 999, monthsToShow: 3} );	
			...
			var data = {
	 		...
	 		jQuery.post( ajaxurl, data, function(response) {	
	 				...
	 				if ( response != -1 && response != -2 && response != -3 && response != -4 && response != -5 && response != 0 ){
						...
						if( objdata.phone_format != '' ){
							jQuery('#wbk-appointment-phone').mask(objdata.phone_format);
						}
