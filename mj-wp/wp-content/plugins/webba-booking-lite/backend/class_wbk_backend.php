...
class WBK_Backend {
	...
	public	function settings_updated() {
		if( isset($_GET['settings-updated']) && $_GET['settings-updated'] ) {
			...
			$time_corr = intval( get_option( 'wbk_email_admin_daily_time', '68400' ));
  			...
  			$diff =  intval( current_time('timestamp', 0)) - intval( current_time('timestamp', 1)) ;

			$timestamp = strtotime('today midnight') + $time_corr - $diff;
	...
	public function inline_upload_enquene(){
		...
		if ( $this->is_edit_page() ) {
			...
			wp_enqueue_script( 'wbk-service-dialog', plugins_url( '/js/wbk-service-dialog.js', __FILE__ )  );
