...
function getAllowedClassNames() {
	return array( 'WBK_Appointments_Table' , 'WBK_Email_Templates_Table' );
}
 	...
 	$result = array();
  	...
  	if( count( $error_fields ) == 0  ){
  	 	...
  	 	if( $edited == 1 ){
  	 		...
  	 		$result['data'] = $new_row;
  	 		$result = json_encode($result);
  	 		$table->onTableAddRow( $wpdb->insert_id );
				...
				foreach ( $row  as  $key => $field ) {				 
					if ( $key == 'id' ){
					    if ( $this->field_set->editable == true ){
					    	$html_control = '<div class="slf_row_control">';
						...
						$component = new $class_name( $title, $name, $value, null );
