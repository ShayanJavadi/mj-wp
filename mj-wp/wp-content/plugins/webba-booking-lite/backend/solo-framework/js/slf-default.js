		...
		jQuery.post( ajaxurl, data, function(response) { 
 			if( response == '1' ){
				...
				jQuery('#' + col_form_id ).find('.slf_table_row_delete').prop('disabled', false);
	 			jQuery('#' + col_form_id ).find('.slf_table_row_cancel').prop('disabled', false);
	 			jQuery('#' + col_form_id ).find('.slf_control_error_message' ).html('Internal error');
...
function slf_set_events(){
	// tablesaw
	aplyTableFiltering();
	jQuery( '.slf_date_range_start, .slf_date_range_end' ).datepicker({dateFormat: "d MM yy" }).on( 'change', function() {
