...
class WBK_Ajax_Controller {
	...
	public function ajaxBook() {
		...
		echo $thanks_message . WBK_PayPal::renderPaymentMethods( $service_id, $appointment_id ) ;
	...
	public function ajaxPreparePayment() {	 
		...
		if ( $paypal->init( wp_get_referer() ) === FALSE ){
			echo 'Payment method not suppoted.';
			wp_die();
			return;
		} 

		$html = $paypal->createPayment( $method, $app_id );

		if ( $html == -1 ){
			echo 'Unkonw payment method.'.$method;
			wp_die();
			return;
		}
		if ( $html == -2 ){
		...
		if ( $html == -3 ){
			echo 'Unable to load service data.';
			wp_die();
			return;
		}
		if ( $html == -4 ){
			echo 'Unable to load appointment data.';
			wp_die();
			return;
		}
		if ( $html == -5 ){
