...
class WC_Bookings_Ajax {
	...
	public function calculate_costs() {
		...
		$display_price    = 'incl' == $tax_display_mode ? $product->get_price_including_tax( 1, $cost ) : $product->get_price_excluding_tax( 1, $cost );
		...
		die( json_encode( array(
			'result' => 'SUCCESS',
			'html'   => apply_filters( 'woocommerce_bookings_booking_cost_string', __( '<span class="class-booking-cost">Booking cost', 'woocommerce-bookings' ), $product ) .  ': '   . "$". $product->price
			. " x " .  $posted['wc_bookings_field_persons'] . ' = <strong>$'.  $display_price . '</strong></span>',
		) ) );
	}
